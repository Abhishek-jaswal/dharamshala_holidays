...
import { Bike, CheckCircle, MessageCircle, Phone, AlertCircle, Zap } from "lucide-react";
            ...
            <AnimatedContainer className="space-y-6">
              <AnimatedItem>
                <h2 className="font-display text-3xl font-bold text-gray-900 mb-5">
                  <AnimatedRevealText text="Explore Dharamshala on a Scooty — Your Way" className="font-display text-3xl font-bold text-gray-900" />
                </h2>
              </AnimatedItem>
              <AnimatedItem>
            ...
            <div className="space-y-6">
              ...
              <div>
                ...
                <div className="space-y-3">
                  {scootyFleet.map((s, i) => (
                    <AnimatedCard key={s.model} delay={i * 0.08} className="group">
                      <div className="bg-gray-50 border border-gray-100 rounded-2xl p-5 flex items-center justify-between hover:border-orange-300 hover:shadow-lg transition-all duration-500">
                        <div>
                          <p className="font-bold text-gray-900 group-hover:text-orange-600 transition-colors">{s.model}</p>
                          <div className="flex gap-2 mt-1">
                            <span className="text-xs bg-orange-50 text-orange-700 px-2 py-0.5 rounded-full">{s.cc}</span>
                        ...
                        <span className="font-bold text-orange-600 text-lg flex items-center gap-1 group-hover:scale-110 transition-transform">
              ...
              {/* Requirements box */}
              <AnimatedCard>
                <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 hover:shadow-md transition-shadow">
          ...
          <AnimatedContainer>
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 mt-8">
              {[
                ...
                <AnimatedCard key={s.place} delay={i * 0.08} className="group">
                  <div className="bg-white rounded-2xl p-5 shadow-sm border border-gray-100 hover:shadow-lg hover:border-orange-300 transition-all duration-500">
                    <h3 className="font-bold text-gray-900 mb-1 group-hover:text-orange-600 transition-colors">{s.place}</h3>
                    <p className="text-sm text-gray-500 mb-2">{s.desc}</p>
                    <span className="text-xs text-orange-600 font-medium">{s.distance}</span>
                  </div>
                </AnimatedCard>
              ))}
            </div>
